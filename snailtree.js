var contractAddress="0x9bfbb38fa974032fa65debacdac73daa4ba13766";
...
function formatEthAdr(adr){
	...
	var _stringLink = '<a href="https://testnet.bscscan.com/address/' + adr + '" target="_blank">' + _smallAdr + '</a>';
	return _stringLink;
}
...
gavepecanEvent.watch(function(error, result){
	if(!error){
		...
		if(checkHash(storetxhash, result.transactionHash) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " gave " + numberWithSpaces(result.args.pecan) + " Eggs to Wonkers, and got " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " ETH in exchange!";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
